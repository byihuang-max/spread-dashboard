...
GAMT_FUNDS = [
    ...
    {"name": "瀚鑫纸鸢量化优选", "code": "SZC020", "strategy": "股票多头-量化选股", "source": "platform"},
    ...
    {"name": "时间序列红利增强1号", "code": "SSV122", "strategy": "股票多头-红利指增", "source": "platform"},
...
def fetch_fund_history(fund_code, start_date="2026-01-01", source="team"):
    """获取单只基金历史净值，支持team/platform"""
    if source == "platform":
...
def fetch_all_funds_history():
    ...
    for fund in GAMT_FUNDS:
        ...
        print(f"正在获取 {fund['name']} ({fund['code']}) [source:{source}] ...")
        history, debug = fetch_fund_history(fund["code"], start_date="2025-01-01", source=source)
...
def save_full_csv(all_histories):
    ...
    if all_dfs:
        ...
        csv_path = "/Users/huangqingmeng/.openclaw/workspace/all_full_nav_data_v2.csv"
        full_df.to_csv(csv_path, index=False, encoding='utf-8')
        print(f"✓ 完整历史数据已保存到: {csv_path}")

def main():
    print("=== 获取完整历史净值v2（补全两只平台数据）===")
    
    fund_results, all_histories = fetch_all_funds_history()
    
    save_full_csv(all_histories)
    
    template_path = "/Users/huangqingmeng/.openclaw/workspace/GAMT_Dashboard_v3_final_20260329_full.html"
    output_path = "/Users/huangqingmeng/.openclaw/workspace/GAMT_Dashboard_final_20260329.html"
