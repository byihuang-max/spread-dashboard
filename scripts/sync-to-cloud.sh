...
if [ $? -eq 0 ]; then
  ...
  ssh -o ConnectTimeout=5 "$REMOTE" 'sudo systemctl restart gamt && bash ~/gamt-dashboard/scripts/fix-perms.sh' 2>/dev/null
...
if [ $? -eq 0 ]; then
  ...
  ssh -p 2222 -o ConnectTimeout=5 "$REMOTE" 'sudo systemctl restart gamt && bash ~/gamt-dashboard/scripts/fix-perms.sh' 2>/dev/null
  echo "[$(date '+%H:%M:%S')] ✅ 后端已重启"
  exit 0
fi
