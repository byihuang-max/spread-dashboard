# Smart Notes 审计报告

> 生成时间: 2026-06-07 20:00
> 笔记总数: 98
> 看板模块数: 35

## 1. 模块覆盖检查

以下 **4** 个看板模块在 Smart Notes 中没有对应笔记:

- [ ] **海外一手要闻** (`overseas_digest`)
- [ ] **宏观Meme叙事** (`macro_meme`)
- [ ] **并购池** (`merger_pool`)
- [ ] **并购深度分析** (`merger_report`)

覆盖率: 31/35 (88%)

## 2. 元数据完整性

### 缺少日期 (2 篇)

- [ ] `smart-notes/notes/research/factors/风格集中度.md`
- [ ] `smart-notes/notes/research/factors/基差情绪.md`

### 缺少标准元数据 (5 篇)
标准字段: 日期、类型、标签

- [ ] `smart-notes/notes/engineering/2026-06-07_研报因子蒸馏FactorForge实现与GAMT接入.md` — 研报因子蒸馏（Factor Forge）实现与接入
- [ ] `smart-notes/notes/engineering/024_数据token与权限补全_通联DataYes_UQER_iFind_Tushare_2026-04-06.md` — 数据 token 与权限补全（通联 DataYes / UQER / iFind / Tushare）
- [ ] `smart-notes/notes/engineering/025_数据源地图_四条数据线与分工_2026-04-06.md` — 数据源地图：四条数据线与分工（2026-04-06）
- [ ] `smart-notes/notes/research/narrative/026_PPI_CPI_流动性回流实体观察框架_2026-04-06.md` — PPI / CPI / 流动性回流实体观察框架（2026-04-06）
- [ ] `smart-notes/notes/research/strategy/2026-06-06_景气度股票池打分逻辑.md` — 景气度股票池打分逻辑

### 缺少看板关联 (93 篇)

- [ ] `smart-notes/notes/decisions/2026-05-31_强势股日报v2改造方向.md` — 强势股日报 v2 改造方向
- [ ] `smart-notes/notes/engineering/2026-05-24_Chart.js_主题适配：覆盖优先级与_afterLayout_兜底.md` — Chart.js 主题适配：覆盖优先级与 afterLayout 兜底
- [ ] `smart-notes/notes/engineering/2026-04-12_经济基本面模块MCP落地说明.md` — 2026-04-12 经济基本面模块 MCP 落地说明
- [ ] `smart-notes/notes/engineering/2026-05-06_GAMT登录框自动登录前抖动修复.md` — GAMT登录框自动登录前抖动修复
- [ ] `smart-notes/notes/engineering/2026-03-08_API密钥配置检查.md` — 2026-03-08 API 密钥配置全面检查
- [ ] `smart-notes/notes/engineering/telegram-multi-bots.md` — 配置多个 Telegram AI 员工
- [ ] `smart-notes/notes/engineering/2026-06-06_GAMT看板工具组内嵌改造与Nginx权限根因修复.md` — GAMT看板工具组内嵌改造与Nginx权限根因修复
- [ ] `smart-notes/notes/engineering/004_腾讯云服务器git配置丢失导致定时任务无法推送.md` — 腾讯云服务器git配置丢失导致定时任务无法推送
- [ ] `smart-notes/notes/engineering/stock_monitor_skill_design.md` — 个股监控 Skill 完整设计记录（2026-05-10）
- [ ] `smart-notes/notes/engineering/2026-05-24_Git_rebase_中_ourstheirs_语义反转踩坑记录.md` — Git rebase 中 ours/theirs 语义反转踩坑记录
- [ ] `smart-notes/notes/engineering/2026-05-19_Git_仓库瘦身与三端同步架构.md` — Git 仓库瘦身与三端同步架构
- [ ] `smart-notes/notes/engineering/2026-04-25_GAMT模块产品化路径_v2.md` — GAMT 商业化修正方向与产品化路径
- [ ] `smart-notes/notes/engineering/2026-05-26_Signal-bar_组件格式规范.md` — Signal-bar 组件格式规范
- [ ] `smart-notes/notes/engineering/2026-05-24_Bloomberg_终端风主题部署架构.md` — 2026-05-24_Bloomberg_终端风主题部署架构
- [ ] `smart-notes/notes/engineering/腾讯云自动部署机制.md` — 腾讯云自动部署机制
- [ ] `smart-notes/notes/engineering/2026-06-07_研报因子蒸馏FactorForge实现与GAMT接入.md` — 研报因子蒸馏（Factor Forge）实现与接入
- [ ] `smart-notes/notes/engineering/024_数据token与权限补全_通联DataYes_UQER_iFind_Tushare_2026-04-06.md` — 数据 token 与权限补全（通联 DataYes / UQER / iFind / Tushare）
- [ ] `smart-notes/notes/engineering/2026-05-23_Nova_Demo_8问题_GAMT_10改进方向.md` — Nova Demo 8问题 + GAMT 10改进方向（大白话版）
- [ ] `smart-notes/notes/engineering/2026-05-24_Bloomberg_终端风主题部署架构_v2.md` — GAMT 看板 Bloomberg 主题 CSS 架构
- [ ] `smart-notes/notes/engineering/2026-03-11_腾讯云部署踩坑实录.md` — 腾讯云部署 — 踩坑实录与最终方案
- [ ] `smart-notes/notes/engineering/Smart_Notes架构改造_动态加载方案.md` — Smart Notes架构改造：动态加载方案
- [ ] `smart-notes/notes/engineering/GAMT看板工作流与Git操作指南.md` — GAMT 看板工作流与 Git 操作指南
- [ ] `smart-notes/notes/engineering/2026-06-04_GAMT_看板_6月3-4日工程修复与批次时序优化.md` — GAMT 看板 6月3-4日工程修复与批次时序优化
- [ ] `smart-notes/notes/engineering/2026-05-05_我们一起经历的这一天.md` — 2026-05-05 · 我们一起经历的这一天
- [ ] `smart-notes/notes/engineering/2026-05-10_邮件订阅者管理工程流程事故复盘.md` — 邮件订阅者管理工程流程事故复盘
- [ ] `smart-notes/notes/engineering/2026-04-17_三端对齐操作手册.md` — 三端对齐操作手册
- [ ] `smart-notes/notes/engineering/2026-05-12_Git_冲突标记潜伏导致的连环故障.md` — Git 冲突标记潜伏导致的连环故障
- [ ] `smart-notes/notes/engineering/2026-05-24_CSS_兜底覆盖与_inline_颜色保留的可靠模式.md` — CSS 兜底覆盖与 inline 颜色保留的可靠模式
- [ ] `smart-notes/notes/engineering/2026-05-10_邮件订阅者管理规则v3-事故驱动最终版.md` — 邮件订阅者管理规则v3-事故驱动最终版
- [ ] `smart-notes/notes/engineering/2026-04-12_自选股跟踪器模块数据源清单.md` — 自选股跟踪器：模块数据源清单（2026-04-12）
- [ ] `smart-notes/notes/engineering/2026-05-19_Nginx_文件权限_404_问题修复记录.md` — Nginx 文件权限 404 问题修复记录
- [ ] `smart-notes/notes/engineering/2026-04-25_GAMT模块产品化路径.md` — 2026-04-25_GAMT模块产品化路径
- [ ] `smart-notes/notes/engineering/2026-05-10_源码打包前必须确认_admin_权限模块.md` — 2026-05-10_源码打包前必须确认_admin_权限模块
- [ ] `smart-notes/notes/engineering/2026-05-08_业务数据保护与三端对齐口径.md` — 业务数据保护与三端对齐口径
- [ ] `smart-notes/notes/engineering/2026-05-09_登录遮罩Bug修复.md` — 2026-05-09_登录遮罩Bug修复
- [ ] `smart-notes/notes/engineering/2026-05-13_Git冲突标记导致登录遮罩残留三次事故复盘.md` — Git冲突标记导致登录遮罩残留三次事故复盘
- [ ] `smart-notes/notes/engineering/2026-04-12_GAMT用户库并表与主库统一.md` — 2026-04-12 GAMT 用户库并表与主库统一
- [ ] `smart-notes/notes/engineering/2026-05-24_inject_脚本重复注入_bug_与_marker_防御原则.md` — inject 脚本重复注入 bug 与 marker 防御原则
- [ ] `smart-notes/notes/engineering/2026-04-25_GAMT主页Bloomberg风格改造与sed教训_v2.md` — Bloomberg主题CSS三层架构复盘与重构方案
- [ ] `smart-notes/notes/engineering/033_全球金融日历模块v1.0开发记录_2026-05-05.md` — 全球金融日历模块 v1.0 开发记录
- [ ] `smart-notes/notes/engineering/2026-04-25_GAMT主页Bloomberg风格改造与sed教训.md` — GAMT 主页 Bloomberg 风格改造与 sed 教训
- [ ] `smart-notes/notes/engineering/025_数据源地图_四条数据线与分工_2026-04-06.md` — 数据源地图：四条数据线与分工（2026-04-06）
- [ ] `smart-notes/notes/engineering/2026-05-07_research-capture_skill_设计决策与技术架构.md` — research-capture skill 设计决策与技术架构
- [ ] `smart-notes/notes/engineering/2026-05-31_GAMT投研看板仓库架构全景.md` — GAMT投研看板仓库架构全景
- [ ] `smart-notes/notes/engineering/2026-06-07_多Bot并行的记忆隔离与共享工作状态层.md` — 多Bot并行的记忆隔离与共享工作状态层
- [ ] `smart-notes/notes/engineering/2026-03-11_腾讯云部署看板迁移指南.md` — 腾讯云部署 - GAMT 看板迁移指南
- [ ] `smart-notes/notes/engineering/Smart_Notes架构改造_动态加载方案_v2.md` — Smart Notes 前端自动同步机制设计
- [ ] `smart-notes/notes/engineering/2026-05-06_微信公众号文章抓取失败的应对方案.md` — 微信公众号文章抓取失败的应对方案
- [ ] `smart-notes/notes/engineering/2026-05-24_全站_emoji_清理两次踩坑与铁律.md` — 全站 emoji 清理两次踩坑与铁律
- [ ] `smart-notes/notes/engineering/2026-05-30_GAMT动态明暗切换完整架构.md` — GAMT 看板动态明暗切换完整架构（2026-05-30 最终版）
- [ ] `smart-notes/notes/research/factors/2026-03-11_强势股情绪择时回测.md` — 强势股情绪择时回测
- [ ] `smart-notes/notes/research/factors/择时回测方法论.md` — 择时回测方法论
- [ ] `smart-notes/notes/research/factors/2026-03-01_量化超额五因子框架.md` — 2026-03-01 量化超额五因子框架
- [ ] `smart-notes/notes/research/factors/2026-03-10_量化因子体系深度讨论.md` — 2026-03-10 量化因子体系深度讨论
- [ ] `smart-notes/notes/research/factors/2026-05-08_A股Smart_Beta风格环境系统框架.md` — A股Smart Beta风格环境系统框架
- [ ] `smart-notes/notes/research/factors/Barra_CNE6风格因子.md` — Barra CNE6 风格因子
- [ ] `smart-notes/notes/research/factors/2026-03-01_五因子嵌套而非平行.md` — 决策：五因子用嵌套结构而非平行加权
- [ ] `smart-notes/notes/research/factors/情绪量化.md` — 情绪量化
- [ ] `smart-notes/notes/research/factors/Alpha158_量价因子库.md` — Alpha158 量价因子库
- [ ] `smart-notes/notes/research/factors/2026-03-11_强势股择时单因子检验结论.md` — 2026-03-11 强势股择时单因子检验结论
- [ ] `smart-notes/notes/research/factors/风格集中度.md` — 风格集中度
- [ ] `smart-notes/notes/research/factors/2026-05-08_A股Smart_Beta风格Proxy定稿口径.md` — A股Smart Beta风格Proxy定稿口径
- [ ] `smart-notes/notes/research/factors/离散度.md` — 离散度
- [ ] `smart-notes/notes/research/factors/2026-05-09_量化私募超额衰减的结构性与出路.md` — 量化私募超额衰减的结构性与出路
- [ ] `smart-notes/notes/research/factors/300指增回测框架.md` — 300指增回测框架
- [ ] `smart-notes/notes/research/factors/涨停指数信号框架v3.md` — 涨停指数信号框架 v3 — 三重交叉验证
- [ ] `smart-notes/notes/research/factors/基差情绪.md` — 基差情绪
- [ ] `smart-notes/notes/research/factors/封单额轧差_抄底指标.md` — 涨跌停封单额轧差 (Seal Spread) — A股抄底指标
- [ ] `smart-notes/notes/research/factors/强势股择时因子全景.md` — 强势股择时因子全景
- [ ] `smart-notes/notes/research/narrative/2026-03-04_MEME化趋势与完整监控系统.md` — MEME化趋势与完整监控系统
- [ ] `smart-notes/notes/research/narrative/叙事-价格背离检测.md` — 叙事-价格背离检测
- [ ] `smart-notes/notes/research/narrative/2026-03-05_叙事监控系统升级-价格背离检测.md` — 叙事监控系统升级：价格背离检测
- [ ] `smart-notes/notes/research/narrative/2026-03-05_宏观Meme叙事生命周期框架.md` — 宏观 Meme 叙事生命周期框架
- [ ] `smart-notes/notes/research/narrative/026_PPI_CPI_流动性回流实体观察框架_2026-04-06.md` — PPI / CPI / 流动性回流实体观察框架（2026-04-06）
- [ ] `smart-notes/notes/research/strategy/2026-05-10_强势股日报选股逻辑映射同花顺公式.md` — 强势股日报选股逻辑映射同花顺公式
- [ ] `smart-notes/notes/research/strategy/2026-03-01_强势股模块讨论.md` — 2026-03-01 强势股模块讨论
- [ ] `smart-notes/notes/research/strategy/2026-04-06_行业财报风险系统升级为双轴评分体系.md` — 行业财报风险系统升级为双轴评分体系
- [ ] `smart-notes/notes/research/strategy/2026-03-28_盘后复盘引擎设计决策.md` — 盘后复盘引擎——盘中涨停因子的解释层
- [ ] `smart-notes/notes/research/strategy/2026-03-01_商品CTA_PCA模块讨论.md` — 2026-03-01 商品CTA PCA模块讨论
- [ ] `smart-notes/notes/research/strategy/CTA波动率因子与归因.md` — CTA趋势策略的波动率因子
- [ ] `smart-notes/notes/research/strategy/2026-03-07_美股风险监控系统设计.md` — 美股风险监控系统设计与实现
- [ ] `smart-notes/notes/research/strategy/2026-04-13_财报风险页多年序列与单季度口径修正.md` — 财报风险页多年序列与单季度口径修正
- [ ] `smart-notes/notes/research/strategy/2026-03-06_仪表盘打分系统优化方案.md` — 仪表盘打分系统优化方案
- [ ] `smart-notes/notes/research/strategy/信息流式交易看板vs策略看板.md` — 信息流式交易看板 vs FOF 策略看板
- [ ] `smart-notes/notes/research/strategy/2026-03-01_CTA归因与波动率讨论.md` — CTA产品归因与波动率讨论
- [ ] `smart-notes/notes/research/strategy/2026-04-12_财报双地图目标与数据底座确认.md` — 财报双地图目标与数据底座确认
- [ ] `smart-notes/notes/research/strategy/2026-03-05_高盛HALO策略观察框架.md` — 高盛 HALO 策略观察框架
- [ ] `smart-notes/notes/research/strategy/2026-04-04_财报风险因子可视化页面设计.md` — 财报风险因子可视化页面设计
- [ ] `smart-notes/notes/research/strategy/2026-05-10_美股泡沫监控看板设计方案.md` — 美股泡沫监控看板设计方案
- [ ] `smart-notes/notes/research/strategy/商品CTA_PCA环境诊断.md` — 商品CTA PCA环境诊断
- [ ] `smart-notes/notes/research/strategy/HALO_重资产范式转移.md` — HALO 策略：重资产范式转移
- [ ] `smart-notes/notes/research/strategy/024_财报雷票系统与行业ETF基本面写照是一体两面_2026-04-04.md` — 财报雷票系统与行业ETF基本面写照是一体两面
- [ ] `smart-notes/notes/research/strategy/2026-06-06_景气度股票池打分逻辑.md` — 景气度股票池打分逻辑

## 3. 陈旧笔记 (>60天未更新)

无陈旧笔记。

## 4. 空分类目录

无空目录。

## 5. 近14天对话中可能未沉淀的决策

共 **5** 条疑似未沉淀:

- [ ] `2026-05-25.md` L121: ### 实施路径（B 方案）
- [ ] `2026-05-30.md` L69: ## 19:20 - Roni 要求画 CSS 架构完整流程图
- [ ] `2026-05-31.md` L3: ## 13:21 - GAMT 仓库架构全景梳理
- [ ] `2026-05-31.md` L29: ## 15:22 - 量化方案节奏确认
- [ ] `2026-06-02.md` L3: ## 拥挤度看板：量价矩阵新增龙头案例 + 涨跌停家数醒目显示（已上线）

*注: 模糊匹配，可能有误报。标记为已处理的可忽略。*

---

**待处理项总计: 109**

待补项较多，建议优先处理模块覆盖和缺日期的笔记。