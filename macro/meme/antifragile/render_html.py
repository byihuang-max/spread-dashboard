...
EQUITY_INDICES = ['纳斯达克100', '标普500', '恒生科技ETF', '科创50ETF', '日经225', '韩国KOSPI']
COMPARISON_ASSETS = ['COMEX黄金', 'WTI原油', 'BTC', '美元兑日元']
MAIN_ASSETS = ['纳斯达克100', '标普500', '恒生科技ETF', '科创50ETF', 'BTC', '日经225', '韩国KOSPI', 'COMEX黄金']

COLORS = {
    '纳斯达克100': '#2563eb', '标普500': '#06b6d4', '恒生科技ETF': '#10b981', '科创50ETF': '#f59e0b',
...
def calc_median_nav(nav_data, window_days=365):
    ...
    equity_cols = [c for c in EQUITY_INDICES if c in df_norm.columns]
    df_norm['股票中位数'] = df_norm[equity_cols].median(axis=1)
    return df_norm
...
(function(){{
  ...
  s.textContent = `
    ...
    .signal-bar{{background:#0f0f0f!important;border:1px solid #2a2a2a!important;border-radius:0!important;border-left:3px solid #ff8c00!important}}
    .signal-main{{color:#ff8c00!important}}
    ...
    thead th{{color:#ff8c00!important;border-color:#2a2a2a!important}}
    ...
    td[style*="background:rgb("]{{opacity:0.7!important;border-color:#1a1a1a!important}}
  ...
  if(!hookCharts()){{var i=setInterval(function(){{if(hookCharts())clearInterval(i)}},200);}}

  var hideLabels = ['纳斯达克100','标普500','恒生科技ETF','科创50ETF','日经225','韩国KOSPI'];
  function hideGrayInMedChart(){{
    if(typeof Chart==='undefined' || !Chart.instances) return false;
    var instances = Object.values(Chart.instances);
    var medChart = null;
    for(var k=0;k<instances.length;k++){{
      if(instances[k].canvas && instances[k].canvas.id==='medianChart'){{medChart=instances[k];break;}}
    }}
    if(!medChart) return false;
    medChart.data.datasets.forEach(function(ds){{
      if(hideLabels.indexOf(ds.label)>=0) ds.hidden = true;
    }});
    medChart.update('none');
    return true;
  }}
  setTimeout(function(){{if(!hideGrayInMedChart()){{var j=setInterval(function(){{if(hideGrayInMedChart())clearInterval(j)}},500);}}}},1000);
}})();
</script>
</body></html>"""
